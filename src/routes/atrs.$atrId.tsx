...
function reportHasPendingAttachmentPayload(report?: AtrReport | null): boolean {
  if (!report) return false;
  const topLevelPending = (report.attachments ?? []).some((a) => !attachmentHasPayload(a));
  if (topLevelPending) return true;
  return (report.actions ?? []).some((action) =>
    (action.evidenceFiles ?? []).some((file) => !attachmentHasPayload(file)),
  );
}
